...
<template>
    <div>
        ...
        <div class="flex items-center input" :class="props.error ? 'input-error' : 'input-primary'">
            <Icon v-if="props.icon" :icon="props.icon" class="pl-2 text-lg" />
            <input :id="id" v-bind="$attrs" :value="modelValue" @input="handleInput" class=" px-2 outline-none w-full h-10" >
        </div>
    </div>
</template>