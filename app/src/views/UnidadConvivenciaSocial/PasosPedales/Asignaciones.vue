<script setup>
    ...
    import { useAsignacionStore } from '@/stores/UnidadConvivenciaSocial/PasosPedales/asignacion'
    ...
    <Modal :open="store.modal.view" header="Expediente" icon="bell-concierge" size="max-w-3xl" >
        ...
        <ol class="flex justify-around">
            ...
            <li
                ...
                <Icon icon="xmark" class="text-2xl" />
                <p>Rechazar</p>
            </li>
        </ol>
        <hr>
        <div>
            ...
            <div v-if="store.option == 2">
                ...
                <fieldset class="border-2 p-4 rounded-lg dark:border-blue-500">
                    <legend class="px-3 text-blue-500">ESPACIO DISPONIBLE</legend>
                    <div class="grid lg:grid-cols-2 gap-4">
                        <div class="col-span-2">
                            <Select 
                                v-model="store.expediente.sede_id" 
                                label="Sedes" 
                                icon="map-location-dot"
                                :options="store.sedes"
                                placeholder="Seleccione sede"
                                return-type="value"
                                :error="hasErrorField(store.errors.asignar,'sede_id')"
                            />
                        </div>
                        <Input v-model="store.expediente.ancho" label="Ancho (mts)" type="number" min="1" icon="ruler-vertical" :error="hasErrorField(store.errors.asignar,'ancho')" />
                        <Input v-model="store.expediente.largo" label="Largo (mts)" type="number" min="1" icon="ruler-horizontal" :error="hasErrorField(store.errors.asignar,'largo')" />
                        <div class="col-span-2">
                            <Text-Area v-model="store.expediente.descripcion" label="Descripción" rows="4" />
                        </div>
                    </div>
                    <Validate-Errors :errors="store.errors.asignar" v-if="store.errors.asignar != 0" />
                </fieldset>
                <br>
                <div class="flex justify-evenly">
                    <Button @click="store.assignSpace" text="Asignar espacio" icon="check" class="btn-light" :loading="store.loading.changeState" />
                </div>
            </div>
            ...
            <div v-if="store.option == 4">
                <Text-Area v-model="store.expediente.latest_workflow.observacion" label="Observación" rows="6" :error="hasErrorField(store.errors.rechazar,'observacion')" />
                <br>
